...
import org.nanoboot.powerframework.simplicity.window.WindowSizeMode;
...
public class GameWindow extends ColorShapesAbstractWindow {
    ...
    public GameWindow() {
        ...
        Task task = new Task("Check for game end", () -> {
            ...
            while (!closed) {
                ...
                if (!closed) {
                    Platform.runLater(() -> {
                        if (ColorShapes.getServices().getAuthenticationService().isLoggedIn()) {
                            MessageBox.showBox(ColorShapes.getServices().getPlayerService().getPlayerNick(), ColorShapes.getServices().getLanguageService().getTextConstant(30001) + this.myScore.getText());
                        }
                        ColorShapes.getServices().getGameService().playNewGame();
                    });
                    gameEnd = false;
                }
            }
    ...
    public void initAreaForUserInteraction() {
        ...
        this.setMinHeight(60 * Simplicity.getDpmm());
        this.setMinWidth(80 * Simplicity.getDpmm());
        sLayout = new SLayout();
        ...
        myNick = new SLabel(ColorShapes.getServices().getPlayerService().getPlayerNick());
    ...
    public void updateBoard(int height, int width) {
        this.board = new Board(height, width, this.boardContainer);
    ...
    public void updateNextBoard(int width) {
        ...
        this.nextBoard = new Board(1, width, this.nextBoardContainer);
        for (int i = 1; i <= width; i++) {
            this.nextBoard.getField(1, i).setIgnoreClickActions(true);
        }
        ...
        double fieldDimension = 100 / width;
        columnConstraints.setPercentWidth(fieldDimension);
    ...
    private void initMenu() {
        ...
        menuBar1.getStylesheets().addAll("css/menu.css", "css/menu" + ColorShapes.getView().getWindowColourSkin().getEnumColour().getNumber() + ".css");
        menuBar2.getStylesheets().addAll("css/menu.css", "css/menu" + ColorShapes.getView().getWindowColourSkin().getEnumColour().getNumber() + ".css");

        gameMenu = new Menu(ColorShapes.getServices().getLanguageService().getTextConstant(1001));
        optionsMenu = new Menu(ColorShapes.getServices().getLanguageService().getTextConstant(1101));
        helpMenu = new Menu(ColorShapes.getServices().getLanguageService().getTextConstant(1201));

        menuBar1.getMenus().addAll(gameMenu, optionsMenu);
        menuBar2.getMenus().addAll(helpMenu);

        newWithCurrentCompositionMenuItem = new MenuItem(ColorShapes.getServices().getLanguageService().getTextConstant(1011));
        newWithCustomCompositionMenuItem = new MenuItem(ColorShapes.getServices().getLanguageService().getTextConstant(1012));
        saveMenuItem = new MenuItem(ColorShapes.getServices().getLanguageService().getTextConstant(1013));
        loadMenuItem = new MenuItem(ColorShapes.getServices().getLanguageService().getTextConstant(1014));
        SeparatorMenuItem gameSeparatorMenuItem = new SeparatorMenuItem();
        logOutMenuItem = new MenuItem(ColorShapes.getServices().getLanguageService().getTextConstant(1021));
        exitMenuItem = new MenuItem(ColorShapes.getServices().getLanguageService().getTextConstant(1022));
        ...
        analysisMenuItem = new MenuItem(ColorShapes.getServices().getLanguageService().getTextConstant(1111));
        nextBallsMenuItem = new MenuItem(ColorShapes.getServices().getLanguageService().getTextConstant(1113));
        playerSettingsMenuItem = new MenuItem(ColorShapes.getServices().getLanguageService().getTextConstant(1115));
        SeparatorMenuItem optionsSeparatorMenuItem = new SeparatorMenuItem();
        screenModeMenuItem = new MenuItem(ColorShapes.getServices().getLanguageService().getTextConstant(1121));
        ...
        helpMenuItem = new MenuItem(ColorShapes.getServices().getLanguageService().getTextConstant(1201));
        SeparatorMenuItem helpSeparatorMenuItem = new SeparatorMenuItem();
        aboutThisGameMenuItem = new MenuItem(ColorShapes.getServices().getLanguageService().getTextConstant(1211));
    ...
    public void startNewWithCustomGameComposition() {
        CustomCompositionWindow customCompositionWindow = new CustomCompositionWindow(ColorShapes.getServices().getGameCompositionService().getGameComposition());
        customCompositionWindow.show();
    }

    private void handleSaveMenuItemAction(ActionEvent event) {
        this.close();
        ColorShapes.getServices().getAuthenticationService().logOut();
    }
    ...
    private void switchToFullScreenModeWindowMode() {
        if (this.getWindowSizeMode() == WindowSizeMode.FULLSCREEN) {
            ...
            this.screenModeMenuItem.setText(ColorShapes.getServices().getLanguageService().getTextConstant(1121));
            if (this.board != null) {
                this.board.updateSize();
            }
        } else {
            this.switchToFullScreen();
            this.screenModeMenuItem.setText(ColorShapes.getServices().getLanguageService().getTextConstant(1122));
            {
                if (this.board != null) {
                    this.board.updateSize();
                }
...
// Show the help window when the F1 key is pressed
        switch (keyCode) {
            ...
            case F11:
                ...
                break;
            case F12:
                TempCommandWindow tempCommandWindow = new TempCommandWindow();
    ...
    public void onResizingWindow() {
        ...
        if (sLayout != null) {
            int sLayoutHeight = (int) (this.getHeight() - this.outerHBox.getHeight() - 4.5 * Simplicity.getDpmm());
        ...
        if (nextBoard != null) {
            this.nextBoard.setMinWidth(nextBoard.getField(1, 1).getMinHeight() * nextBoard.getWidthCount());
            this.nextBoard.setMaxWidth(nextBoard.getHeight() * nextBoard.getWidthCount());
//this.nextBoard.minWidthProperty().bind(nextBoard.getField(1,1).heightProperty().multiply(nextBoard.getWidthCount()));
        }
    }
    ...
    public Field getField(int column) {
        return this.nextBoard.getField(1, column);
    }
